...
import { fetchHubs } from '../../utils/api';
import './FilterDropdown.css';

const HubFilter = ({ selectedHubs = [], setSelectedHubs }) => {
  ...
  const toggleHubSelection = (hubValue) => {
    const hub = hubOptions.find(h => h.value === hubValue);
    if (!hub) return; // Ensure valid selection

    if (selectedHubs.some(h => h.value === hub.value)) {
      setSelectedHubs(selectedHubs.filter((h) => h.value !== hub.value));
    } else {
  ...
  return (
    <div className="mt-6 mb-6">
      <p className="text-base font-[500] font-poppins mb-2">Select Hub</p>
          ...
          <div className="flex flex-wrap gap-2">
            {selectedHubs.length > 0 ? (
              selectedHubs.map((hub) => (
                <div key={hub.value} className="flex items-center bg-[#F1F1F1] text-black rounded-sm py-1 px-4 text-sm">
                  {hub.label}
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
            ...
            <div className="flex flex-col p-2">
              {hubOptions.map((option) => (
                <label key={option.value} className="flex items-center mb-2">
                  <input
                    type="checkbox"
                    checked={selectedHubs.some(hub => hub.value === option.value)}
                    onChange={() => toggleHubSelection(option.value)}
