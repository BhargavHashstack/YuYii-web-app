...
const MonthFilter = ({ selectedMonths = [], setSelectedMonths }) => {
  const months = [
  ...
  return (
    <div className="mb-6">
      <p className="text-base font-[500] font-poppins mb-2">Select Month</p>
          ...
          <div className="flex flex-wrap gap-2">
            {selectedMonths.length > 0 ? (
              selectedMonths.map((month) => (
                <div
                  key={month}
                  className="flex items-center bg-[#F1F1F1] text-black rounded-sm py-1 px-2 text-sm"
                >
                  {month}
                  <button
                    onClick={(e) => {
              ...
              {months.map((option) => (
                <label key={option.value} className="flex items-center mb-2">
                  <input
                    ...
                    onChange={() => toggleMonthSelection(option.value)}
