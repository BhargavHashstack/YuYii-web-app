...
const CityFilter = ({ selectedCities = [], setSelectedCities }) => {
  ...
  useEffect(() => {
    const loadCities = async () => {
      try {
        ...
        setCityOptions(Array.isArray(cities) ? cities.map(city => ({
          value: city.value,
          label: city.label,
        })) : []); // ✅ Ensure it's always an array
      } catch (error) {
        console.error("❌ Error fetching cities:", error);
        setCityOptions([]); // ✅ Fallback to an empty array
      }
    };
  ...
  return (
    <div className="mb-6">
      <p className="text-base font-[500] font-poppins mb-2">Select City</p>
